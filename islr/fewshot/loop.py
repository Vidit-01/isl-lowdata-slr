...
from islr.common import CACHE_DIR, CHECKPOINT_DIR, WEIGHTS_DIR, save_json
from islr.common.engine import append_train_log, evaluate, save_weights, train_one_epoch
...
from islr.models.registry import BaselineSpec, get_spec
...
def make_loaders(
    ...
    if spec.modality in ("skeleton_kdf", "parts_kdf"):
        kdf_kw = dict(kw)
        ...
        def _mk_kdf(df, augment: bool):
            return KDFSkeletonDataset(
                ...
                augment=augment,
                require_cache=True,
                full_seq=spec.modality == "parts_kdf",
            )
